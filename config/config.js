...
var home = process.env.INSIGHT_DB || (getUserHome() + '/.insight-bux');
...
var forceRPCsync = 0; //process.env.INSIGHT_FORCE_RPC_SYNC;
...
if (!dataDir) {
  if (isWin) dataDir = '%APPDATA%\\Buxcoin\\';
  if (isMac) dataDir = process.env.HOME + '/Library/Application Support/Buxcoin/';
  if (isLinux) dataDir = process.env.HOME + '/.Buxcoin/';
}
