...
var genesisTXID = '000000a49664091ce50794d84fb243cde97738534b554b2022fa00a029d0ab7c'; //ParkByte
var CONCURRENCY = 5;
...
var encodedData = require('soop').load('buxcore/util/EncodedData', {
  base58: base58
});
var versionedData = require('soop').load('buxcore/util/VersionedData', {
  parent: encodedData
});

var Address = require('soop').load('buxcore/lib/Address', {
...
// Proof of Stake - Gets address from vout[1] and matches against /etc/minersPoolStrings.json
TransactionDb.prototype.getPoolInfo = function(txid, cb) {
  ...
  Rpc.getTxInfo(txid, function(err, txInfo) {
    ...
    var ret;
    if (txInfo && txInfo.vout[1])
      ret = self.poolMatch.match(new Buffer(txInfo.vout[1].scriptPubKey.addresses[0]));
    return cb(ret);
  });
};

// Proof of Work - Gets coinbase and matches against /etc/minersPoolStrings.json
TransactionDb.prototype.getPoolInfoPoW = function(txid, cb) {
  var self = this;
